...
std::vector<uint8_t> BS::Crypto::compute_sha_224(std::span<const uint8_t> bytes) {

    std::vector<uint8_t> result(SHA224_DIGEST_LENGTH);

    SHA224(bytes.data(), bytes.size(), result.data());
