...
std::string BS::Encoding::encode_base64_url_no_padding(std::span<const uint8_t> bytes) {
...
std::optional<std::vector<uint8_t>> BS::Encoding::decode_base64_any(std::string_view s)
{
    tl_error = false;
    ...
    std::vector<uint8_t>::size_type output_size =
            s.size() / 4 * 3 +
            s.size() % 4 - 1;
...
std::string BS::Encoding::encode_hex_lower(std::span<const uint8_t> bytes)
{
    std::string result;
...
std::string BS::Encoding::encode_hex_upper(std::span<const uint8_t> bytes)
{
    std::string result;
    ...
    for (const auto b : bytes)
    {
        result.push_back(c_a16u[b >> 4]);
        result.push_back(c_a16u[b & 0xF]);
    }
...
std::optional<std::vector<uint8_t>> BS::Encoding::decode_hex_any(std::string_view s)
{
    tl_error = false;
